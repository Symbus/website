...
async function loadSteamProfile(steamId) {
  ...
  try {
    /*
      PlayerDB is used here because a static GitHub Pages site cannot safely
      expose a Steam Web API key, and Steam's Web API is not suitable for a
      direct browser request in this setup.

      If PlayerDB is unavailable, the loading screen still works and simply
      falls back to a generic player card.
    */
    ...
    if (!response.ok) {
      throw new Error(`PlayerDB returned ${response.status}`);
    }
